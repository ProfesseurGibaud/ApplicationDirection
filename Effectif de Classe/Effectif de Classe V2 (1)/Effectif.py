...
class MenuScreen(Screen):
    def build(self,n):
        ...
        self.GroupeDeClasses.FaireClasseGene()
        self.GroupeDeClasses.CalculEnergie()
        # On cree le contenu:
        Menu_Layout = GridLayout(padding=20, spacing=10, cols = 12)
        self.DicoLabel = {}
        TempListe = []
        L = ["Classe","Effectif","Sexe","HLP","Géopol","LCE","SES","Math","Phy","SVT","SI","NSI"]
        LL = ["HLP","Géopol","LCE","SES","Math","Phy","SVT","SI","NSI"]
        for j in range(0,11):
            Menu_Layout.add_widget(Label(text = L[j]))
        for i in range(1,n):
            TempListe.clear()
            nom = "Première Géné " + str(i)
            self.DicoLabel[nom] = Label(text = nom)
            Menu_Layout.add_widget(Label(text = str(self.GroupeDeClasses.DicoClasse[nom].N)))
            HF = self.GroupeDeClasses.DicoClasse[nom].Homme - self.GroupeDeClasses.DicoClasse[nom].Femme
            Menu_Layout.add_widget(Label(text = str(HF)))    # Mettre Sexe
            for j in LL:
                Menu_Layout.add_widget(Label(text = str(self.GroupeDeClasses.DicoClasse[nom].DicoSpe[j])))   # Mettre Les nombres des Spé
