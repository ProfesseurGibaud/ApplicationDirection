...
def FaireClasseur(GroupeDeClasses):
    ...
    for classe in Test1.DicoClasse:
        ...
        Liste = ["Nom", "Prénom", "Genre","HLP","Géopol","LCE","SES","Math","Phy","SVT","SI","NSI"]
        j= 0
        for l in Liste:
            feuille.write(0,j,l)
            j+=1
        ...
        for eleve in Test1.DicoClasse[classe].DicoEleve:
            ...
            for char in DicoSave[eleve]:
                #print(char)
                feuille.write(i,k,DicoSave[eleve][k])
                k = k + 1 
            i = i+1
...
for i in list(Test1.DicoClasse.keys()):
    nom = i
    DicoLabel[nom + "N"] = Label(text = str(Test1.DicoClasse[nom].N))
    HF = Test1.DicoClasse[nom].Homme - Test1.DicoClasse[nom].Femme
    DicoLabel[nom + "HF"] = Label(text = str(HF))
    for j in LL:
        DicoLabel[nom + j] = Label(text = str(Test1.DicoClasse[nom].DicoSpe[j]))
